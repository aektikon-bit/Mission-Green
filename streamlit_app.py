...
cities = pd.DataFrame({
    ...
    'lon': [100.5018,-74.0060,-0.1278,139.6917,151.2093,2.3522,77.2090,31.2357,-43.1729,18.4249],
    'co2_emission': [10.5,15.0,6.0,9.0,4.5,5.5,8.0,3.0,2.5,1.5],
...
selected_city = st.sidebar.selectbox(
    "📍 เมืองที่ต้องการช่วย",
    available_cities['city'] if len(available_cities)>0 else ["-"],
    index=0
)
...
# ฟังก์ชันทำภารกิจ
# -----------------------------
def do_mission(tree_action, co2_action):
    score_gain = int(tree_action*0.02 + co2_action*random.uniform(1,3))
    st.session_state.score += score_gain
    st.session_state.completed_cities.append(st.session_state.selected_city)
    st.success(f"คุณทำภารกิจใน {st.session_state.selected_city} สำเร็จ! ได้คะแนน {score_gain}")

# -----------------------------
# แสดงภารกิจ
# -----------------------------
if len(available_cities) > 0:
    city_data = cities[cities['city']==st.session_state.selected_city].iloc[0]
    st.subheader(f"🌱 ภารกิจใน {st.session_state.selected_city}")
    ...
    if st.button("✅ ทำภารกิจ"):
        do_mission(tree_action, co2_action)
else:
    st.subheader("🎉 คุณทำภารกิจครบทุกเมืองแล้ว!")

# -----------------------------
# แผนที่โลก
# -----------------------------
cities['color'] = cities['city'].apply(
    lambda x: [0,200,0] if x in st.session_state.completed_cities else [255,100,100]
)
